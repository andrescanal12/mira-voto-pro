...
try {
    ...
    dataRaw.forEach(group => {
        group.names.forEach(name => {
            ...
            const finalVoterId = (voterId && validVoterIds.has(voterId)) ? voterId : null;
            values.push(`('${name}', ${finalVoterId ? `'${finalVoterId}'` : 'NULL'}, '${group.cat}')`);
        });
    });

    sql += values.join(",\n") + ";";
    fs.writeFileSync('c:/Users/andre/mivotopro/mira-voto-pro/insert_logistica_v5.sql', sql);
