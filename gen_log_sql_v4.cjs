...
try {
    const db_raw = fs.readFileSync('C:/Users/andre/.gemini/antigravity/brain/c66d302d-2351-4b32-9708-57fcb089155a/.system_generated/steps/2294/output.txt', 'utf8');
    const db_json_text = JSON.parse(db_raw).result.match(/<untrusted-data-[^>]+>\n([\s\S]+?)\n<\/untrusted-data-/)[1];
    const db_data = JSON.parse(db_json_text);

    const validVoterIds = new Set(db_data.map(v => v.id));
    ...
    db_data.forEach(v => {
        ...
        if (!voterMap.has(norm)) {
            voterMap.set(norm, v.id);
        }
    });

    let sql = "TRUNCATE TABLE public.logistica;\n";
    sql += "INSERT INTO public.logistica (nombre_manual, voter_id, categoria) VALUES\n";
    const values = [];
    dataRaw.forEach(group => {
        group.names.forEach(name => {
            const voterId = voterMap.get(normalize(name));
            // DOUBLE CHECK the ID is in the db_data list
            const isReallyValid = validVoterIds.has(voterId);
            values.push(`('${name}', ${isReallyValid ? `'${voterId}'` : 'NULL'}, '${group.cat}')`);
        });
    });

    sql += values.join(",\n") + ";";
    fs.writeFileSync('c:/Users/andre/mivotopro/mira-voto-pro/insert_logistica_v4.sql', sql);
