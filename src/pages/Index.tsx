...
import { BarChart3, Database, Phone, FileDown, CheckCircle2, UserPlus, Search, ExternalLink, Headset, Truck, PieChart } from "lucide-react";
...
type Tab = "dashboard" | "pendientes" | "call_center" | "ya_llamados" | "reportes" | "logistica" | "rendicion";
...
const tabs: { id: Tab; label: string; icon: React.ElementType }[] = [
  ...
  { id: "rendicion", label: "Rendición de Cuentas", icon: PieChart },
];
...
const Index = () => {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${activeTab === "rendicion" ? "bg-[#F4F7FC] text-slate-800" : "mira-bg-pattern"}`} style={{ position: "relative" }}>
      ...
      <div className="relative z-10 min-h-screen">
        ...
        {/* ── MAIN CONTENT ── */}
        <main className={`container mx-auto px-4 py-6 space-y-6 max-w-7xl ${activeTab === "rendicion" ? "pt-12" : ""}`}>
          {/* Nav + indicador de sync */}
          <div>
            {/* Nav + acciones */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
              <nav className={`flex gap-1 rounded-2xl p-1 overflow-x-auto transition-all ${
                activeTab === "rendicion" 
                  ? "bg-white border border-[#E2E8F5]" 
                ...
                {tabs.map((tab) => (
                  <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium whitespace-nowrap transition-all ${
                      activeTab === tab.id
                        ? activeTab === "rendicion"
                          ? "bg-[#00289F] text-white shadow-md"
                          : "bg-primary text-primary-foreground shadow-lg"
                        : activeTab === "rendicion"
                          ? "text-[#475569] hover:text-[#00289F] hover:bg-slate-50"
              ...
              <div className="flex items-center justify-between sm:justify-end gap-3 px-1">
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 text-[12px] font-bold px-3 py-1.5 rounded-xl transition-colors shadow-sm ${
                    activeTab === "rendicion"
                      ? "bg-[#00289F] hover:bg-[#0B3B74] text-white"
          ...
          <div>
            ...
            {activeTab === "rendicion" && <Rendicion onBack={() => setActiveTab("dashboard")} />}
          </div>
        </main>
